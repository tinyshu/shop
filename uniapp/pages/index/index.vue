<template>
	<pageWrapper>
		...
		<!-- 轮播图 -->
		<view class="king-p-5">
			<swiper v-if="banner.length" class="home-banner" indicator-dots circular autoplay interval="3000" duration="500">
				<swiper-item v-for="(item, index) in banner" :key="index">
					<image class="home-banner__image" :src="item.imgUrl" mode="aspectFill"
						@tap="clickBanner(index)" @error="handleBannerError(index)" />
				</swiper-item>
			</swiper>
		</view>
		<!-- 首页分类 -->
		<view class="king-bg-white king-mx-10 king-radius10 king-pb-5 king-pt-10" style="min-height: 80px">
			<u-grid :border="false" col="4">
				<u-grid-item v-for="c in category" :key="c.ID" @click="toGoodsByCategory(c.ID)">
					<u--image width="45" height="45" :src="c.imgUrl" shape="circle" @error="handleCategoryError(c)"></u--image>
...
<script>
	...
	import { resolveImageUrl, onImageError } from '@/utils/image.js'
	import {
		getBannerList
	} from '@/api/banner.js'
	import {
	...
	export default {
		...
		methods: {
			...
			handleBannerError(index) {
				onImageError(this.banner[index], 'imgUrl')
			},
			handleCategoryError(item) {
				onImageError(item, 'imgUrl')
			},
			// 切换标签页
			onChangeGoodsTabs(e) {
			...
			async getGoodsListData(tabId, type) {
				...
				res.data.list.forEach(item => {
					if (item.images[0]) {
						item.images[0].url = resolveImageUrl(item.images[0].url)
					}
				})
				console.log(res);
				// 进行赋值并计算是否还有下一页
